...
async def _search_bilibili_and_build_messages(session_id: str, question: str) -> tuple[list[dict], list[dict]]:
    ...
    try:
        ...
        if "有什么" in question:
            ...
            # 先移除"可以给我"这类模式
            if "可以给我" in search_keyword:
                search_keyword = search_keyword.replace("可以给我", "")
            if "推荐" in search_keyword:
                search_keyword = search_keyword.replace("推荐", "")
            if "视频推荐" in question and "视频" in search_keyword:
                search_keyword = search_keyword.replace("视频", "")

            # 移除其他词汇
            remove_words = ["建议", "介绍", "好看的", "值得看", "能不能", "能否", "帮我", "请", "一下", "一些", "几个", "点", "呀", "呢", "吗", "？", "?"]
        ...
        # 如果WBI搜索失败或无结果，尝试使用热搜词API
        items = []
        if search_result.get("success") and search_result.get("items"):
            items = search_result.get("items", [])
            logger.info(f"WBI搜索成功，获取到 {len(items)} 个结果")
        else:
            # WBI搜索失败，使用热搜词作为备选
            logger.warning(f"WBI搜索失败: {search_result.get('error', 'Unknown error')}，尝试使用热搜词")
            hot_result = await bili.get_hot_search_recommendations(limit=10)
            if hot_result.get("success") and hot_result.get("items"):
                items = hot_result.get("items", [])
                logger.info(f"热搜词获取成功，获取到 {len(items)} 个热搜")

        await bili.close()

        if not items:
            logger.warning("所有搜索方式均无结果")
            return _build_direct_messages(question), []
...
def _is_collection_intent(question: str) -> bool:
    ...
    if _is_recommendation_question(question):
        return False
    terms = ["收藏", "收藏夹", "合集", "up主", "BV", "bv", "分P", "字幕", "知识库", "入库", "���步", "向量", "检索"]
    # 对于"视频"关键词，检查上下文避免误判推荐类问题
    if "视频" in question:
        # 如果单独说"视频"或者与其他收藏相关词汇一起出现，视为收藏意图
        return any(term in question for term in terms) or "我的视频" in question or "我的收藏" in question
    return any(term in question for term in terms)

def _is_recommendation_question(question: str) -> bool:
    """是否是推荐类问题"""
    terms = ["推荐", "建议", "有什么", "介绍", "推荐视频", "视频推荐", "好看的", "值得看"]
    return any(term in question for term in terms)
...
async def _prepare_messages(request: ChatRequest, db: AsyncSession) -> tuple[list[dict], List[dict], str]:
    ...
    # 2) 无数据时处理
    if not has_data:
        # 如果是推荐类问题，尝试调用Bilibili搜索
        if _is_recommendation_question(question):
            logger.info("检测到推荐类问题且无本地数据，尝试Bilibili搜索")
            messages, sources = await _search_bilibili_and_build_messages(request.session_id, question)
