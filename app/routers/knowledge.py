...
async def _sync_folder(
    ...
    # 来源优先级（数值越大优先级越高，字幕>ASR>AI摘要>基本信息）
    source_priority = {
        ContentSource.BASIC_INFO.value: 1,
        ContentSource.AI_SUMMARY.value: 2,
        ContentSource.ASR.value: 3,
        ContentSource.SUBTITLE.value: 4,
    }
    ...
    def _is_subtitle_cache_usable(cache: Optional[VideoCache]) -> bool:
        """判断字幕缓存是否可用"""
        if not cache:
            return False
        if cache.content_source != ContentSource.SUBTITLE.value:
            return False
        text = (cache.content or "").strip()
        return len(text) >= 50
    ...
    for bvid in targets:
        ...
        try:
            ...
            if (global_count == 0) or should_reindex:
                if not content:
                    # 优先使用字幕缓存
                    if _is_subtitle_cache_usable(cache):
                        ...
                        cache.is_processed = True
                        logger.info(f"[{bvid}] 使用缓存字幕内容重建向量")
                    # 其次使用ASR缓存
...
@router.get("/content-fetcher/stats")
async def get_content_fetcher_stats():
    """获取内容获取器统计信息"""
    try:
        from app.services.content_fetcher import ContentFetcher
        stats = ContentFetcher.get_stats()
        return stats
    except Exception as e:
        logger.error(f"获取内容获取统计失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))


@router.post("/content-fetcher/reset-stats")
async def reset_content_fetcher_stats():
    """重置内容获取器统计信息"""
    try:
        from app.services.content_fetcher import ContentFetcher
        ContentFetcher.reset_stats()
        return {"message": "统计信息已重置"}
    except Exception as e:
        logger.error(f"重置内容获取统计失败: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
class RebuildRequest(BaseModel):
    """重建知识库请求"""
    folder_ids: Optional[List[int]] = None  # 指定收藏夹，不指定则处理所有
    force_refresh: bool = True  # 强制刷新已有内容
    strategy: Optional[str] = "auto"  # auto/subtitle/asr/ai_summary
...
async def rebuild_knowledge_base(
    ...
    return {"task_id": task_id, "message": "重建任务已启动"}
...
async def _rebuild_knowledge_base_task(
    ...
    try:
        build_tasks[task_id]["status"] = "running"
        build_tasks[task_id]["current_step"] = "初始化服务..."
            ...
            async with get_db_context() as db:
                ...
                if folder_ids:
                    folders_result = await db.execute(
                        select(FavoriteFolder).where(
                            FavoriteFolder.session_id == session_id,
                            FavoriteFolder.media_id.in_(folder_ids)
                        )
                    )
                else:
                    folders_result = await db.execute(
                        select(FavoriteFolder).where(FavoriteFolder.session_id == session_id)
                    )
                ...
                if not folders:
                    build_tasks[task_id]["status"] = "completed"
                    build_tasks[task_id]["progress"] = 100
                    build_tasks[task_id]["message"] = "没有找到需要处理的收藏夹"
                    return
                ...
                build_tasks[task_id]["total_videos"] = total_videos

                if total_videos == 0:
                    build_tasks[task_id]["status"] = "completed"
                    build_tasks[task_id]["progress"] = 100
                    build_tasks[task_id]["message"] = "没有找到视频"
                    return
                ...
                build_tasks[task_id]["current_step"] = f"开始重建 {total_videos} 个视频..."
                ...
                for bvid in all_bvids:
                    try:
                        ...
                        if not force_refresh:
                            ...
                            current_priority = source_priority.get(cache.content_source or "", 0)
                            if current_priority >= 3:  # ASR或字幕，不需要刷新
                                skipped += 1
                                continue

                        # 使用新的内容获取策略
                        build_tasks[task_id]["current_step"] = f"处理: {cache.title or bvid}"

                        content = await content_fetcher.fetch_content(bvid, cache.cid, cache.title)
                        new_source = content.source.value
                        new_text = (content.content or "").strip()

                        # 检查是否需要更新
                        old_text = (cache.content or "").strip()
                        old_source = cache.content_source

                        should_update = False
                        if not old_text or len(old_text) < 50:
                            should_update = True
                        elif source_priority.get(new_source, 0) > source_priority.get(old_source or "", 0):
                            should_update = True
                        ...
                        if total_videos > 0:
                            build_tasks[task_id]["progress"] = int((processed / total_videos) * 100)

                    except Exception as e:
                        logger.warning(f"重建视频失败 [{bvid}]: {e}")
                        processed += 1
                ...
                build_tasks[task_id]["processed_videos"] = processed
                build_tasks[task_id]["current_step"] = "完成"
                build_tasks[task_id]["message"] = f"重建完成：刷新 {refreshed}，跳过 {skipped}"

                logger.info(f"知识库重建完成: 刷新 {refreshed}，跳过 {skipped}")

                # 显示内容获取统计
                fetcher_stats = ContentFetcher.get_stats()
                logger.info(f"内容获取统计: {fetcher_stats}")

        finally:
            await bili.close()

    except Exception as e:
        logger.error(f"重建任务失败: {e}")
        build_tasks[task_id]["status"] = "failed"
        build_tasks[task_id]["message"] = str(e)


@router.get("/rebuild/status/{task_id}", response_model=BuildStatus)
