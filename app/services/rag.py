...
# Use simple text splitter to avoid spacy/pydantic v1 conflict
try:
    from langchain_text_splitters import RecursiveCharacterTextSplitter
except (ImportError, Exception):
    # Fallback: simple character-based splitter
    class _SimpleTextSplitter:
        ...
        def split_text(self, text):
            ...
            while start < len(text):
                ...
                # Try to find a good separator near the chunk boundary
                if end < len(text):
    ...
    class RecursiveCharacterTextSplitter(_SimpleTextSplitter):
        pass
...
class RAGService:
    ...
    def __init__(self, collection_name: str = "bilibili_videos"):
        ...
        try:
            ...
            logger.warning(f"DashScopeEmbeddings 不可用，使用 OpenAIEmbeddings: {e}")
...
字幕内容："""),
    ...
    def search_with_score(
        ...
        Returns:
            [(Document, score), ...]，score 为负距离分数，越高越相似
        """
        if not query or not query.strip():
            logger.warning("检索查询为空")
            return []
        ...
        try:
            ...
            logger.info(f"检索完成（带分数）：query='{query}'，召回={len(results)}")
            for idx, (doc, score) in enumerate(results):
                meta = doc.metadata or {}
                title = meta.get("title", "")
                bvid = meta.get("bvid", "")
                preview = doc.page_content[:120].replace("\n", " ").strip()
                logger.info(f"召回[{idx+1}] {bvid} {title} | score={score:.4f} | {preview}")

            return results
        except Exception as e:
            logger.warning(f"带分数检索失败: {e}")
            return []
