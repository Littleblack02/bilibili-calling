...
class ContentFetcher:
    ...
    1. 字幕 (SUBTITLE) - 最快最准确
    2. ASR (ASR) - 成本高但覆盖广
    3. AI摘要 (AI_SUMMARY) - 质量高但不稳定
    4. 基本信息 (BASIC_INFO) - 兜底
    """
    ...
    def get_stats(cls) -> Dict[str, Any]:
        """获取内容获取统计信息"""
        total = cls._stats.get("total", 0)
        if total == 0:
            return {
                "total": 0,
                "success_rate": 0.0,
                "strategies": {
                    "subtitle": {"count": 0, "rate": 0.0},
                    "asr": {"count": 0, "rate": 0.0},
                    "ai_summary": {"count": 0, "rate": 0.0},
                    "basic_info": {"count": 0, "rate": 0.0}
                }
            }

        return {
            "total": total,
            "success_rate": (total - cls._stats.get("failed", 0)) / total * 100,
            "strategies": {
                "subtitle": {
                    "count": cls._stats.get("subtitle", 0),
                    "rate": cls._stats.get("subtitle", 0) / total * 100
                },
                "asr": {
                    "count": cls._stats.get("asr", 0),
                    "rate": cls._stats.get("asr", 0) / total * 100
                },
                "ai_summary": {
                    "count": cls._stats.get("ai_summary", 0),
                    "rate": cls._stats.get("ai_summary", 0) / total * 100
                },
                "basic_info": {
                    "count": cls._stats.get("basic_info", 0),
                    "rate": cls._stats.get("basic_info", 0) / total * 100
                }
            }
        }
    ...
    async def fetch_content(self, bvid: str, cid: int = None, title: str = None) -> VideoContent:
        ...
        # 策略2: ASR次之（成本高但覆盖广）
        logger.info(f"[{bvid}] 尝试策略2: ASR音频转写")
        asr_text = await self._try_asr(bvid, cid)
        if asr_text and len(asr_text) >= 50:
            logger.info(f"[{bvid}] ✓ 使用ASR内容 (长度: {len(asr_text)})")
            self._stats["asr"] += 1
            return VideoContent(
                bvid=bvid,
                title=title,
            ...
            logger.info(f"[{bvid}] ASR内容过短 ({len(asr_text)}字符)，跳过")

        # 策略3: AI摘要（可选，质量高但不稳定）
        logger.info(f"[{bvid}] 尝试策略3: AI摘要")
        try:
            ai_summary = await self._try_ai_summary(
                bvid, cid,
                video_info.get("aid") if video_info else None
            )
            if ai_summary:
                summary_text = ai_summary.get("summary", "")
                outline = ai_summary.get("outline", [])
                ...
                if summary_text and len(summary_text) >= 30:
                    ...
                    if outline:
                        ...
                        full_content += outline_text

                    logger.info(f"[{bvid}] ✓ 使用AI摘要 (长度: {len(full_content)})")
                    self._stats["ai_summary"] += 1
                    return VideoContent(
                        bvid=bvid,
                        title=title,
                    ...
                    logger.info(f"[{bvid}] AI摘要过短或为空")
        except Exception as e:
            logger.debug(f"[{bvid}] AI摘要获取失败: {e}")

        # 策略4: 基本信息兜底
        logger.info(f"[{bvid}] 使用策略4: 基本信息兜底")
        ...
        logger.info(f"[{bvid}] ✓ 使用基本信息 (长度: {len(basic_content)})")
        self._stats["basic_info"] += 1
        return VideoContent(
            bvid=bvid,
            title=title,
